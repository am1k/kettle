...
(function (factory) {
  ...
  Stickit.ViewMixin = {
    ...
    // Collection of models event bindings.
    //   [{models,event,fn,config}, ...]
    _modelBindings: null,

    // Unbind the models and event bindings from `this._modelBindings` and
    // `this.$el`. If the optional `models` parameter is defined, then only
    // delete bindings for the given `models` and its corresponding views events.
    unstickit: function(model, bindingSelector) {
      ...
      // Trigger an event for each models that was unbound.
    ...
    // Initilize Stickit bindings for the views. Subsequent binding additions
    ...
    stickit: function(optionalModel, optionalBindingsConfig) {
      ...
      // Wrap `views.remove` to unbind stickit models and dom events.
    ...
    // Add a single Stickit binding or a hash of bindings to the models. If
    // `optionalModel` is ommitted, will default to the views's `models` property.
    addBinding: function(optionalModel, selector, binding) {
      ...
      // Special case the ':el' selector to use the views's this.$el.
      ...
      // Allow shorthand setting of models attributes - `'selector':'observe'`.
      ...
      // Create the models set options with a unique `bindId` so that we
      // can avoid double-binding in the `change:attribute` event handler.
      var bindId = config.bindId = _.uniqueId();

      // Add a reference to the views for handlers of stickitChange events
      ...
      if (modelAttr) {
        // Setup one-way (input element -> models) bindings.
        _.each(config.events, function(type) {
          ...
          var listener = function(event) {
            ...
            // Don't update the models if false is returned from the `updateModel` configuration.
        ...
        // Setup a `change:modelAttr` observer to keep the views element in sync.
  ...
  // If the given `fn` is a string, then views[fn] is called, otherwise it is
  ...
  // Given a function, string (views function reference), or a boolean
  ...
  // Setup a models event binding with the given function, and track the event
  // in the views's _modelBindings.
  ...
  // Prepares the given `val`ue and sets it into the `models`.
  ...
  // Returns the given `attr`'s value from the `models`, escaping and
  ...
  // property `name`, to an `observe`d models attribute, using an optional
  // `onGet` formatter.
  //
  //     attributes: [{
  ...
  // If `visible` is configured, then the views element will be shown/hidden
  // based on the truthiness of the modelattr's value or the result of the
  // given callback. If a `visibleFn` is also supplied, then that callback
  // will be executed to manually handle showing/hiding the views element.
  //
  //     observe: 'isRight',
  ...
  //
  //     update: function($el, val, models, options) {},  // handler for updating
  ...
  Stickit.addHandler([{
    ...
    update: function($el, val, model, options) {
      ...
      // which represents the path to the list relative to `window` or the views/`this`.
      if (_.isString(list)) {
        var context = window;
      ...
      if (optList instanceof Backbone.Collection) {
        ...
        // Compare the views cid on the event to this.cid to ensure we don't
        ...
        // Remove the previous models event listener
